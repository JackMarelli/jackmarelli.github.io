...
_app.drawUnit = (u) => {
  ...
  _app.ctx.fillRect(u.x, u.y, _app.step, _app.step);
};
...
_app.infect = () => {
  for (let i = 0; i < _app.divisions; i++) {
    for (let j = 0; j < _app.divisions; j++) {
      ...
      if (_app.matrix[i][j + 1]) {
        console.log("unit exists");
        if (_app.matrix[i][j].color != _app.matrix[i][j + 1].color) {
          console.log("fight");
          if (_app.matrix[i][j].fight(_app.matrix[i][j + 1])) {
            _app.drawUnit(_app.matrix[i][j]);
          }
        }
      }
